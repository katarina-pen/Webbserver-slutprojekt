...
Ett tips är att använda [`String#split`](https://ruby-doc.org/3.2.2/String.html#method-i-split). Tänk på att radbrytning i HTTP är två tecken (`\r\n`). 
...
<img src="class_diagram.png" alt="klassdiagram" style="zoom: 25%;" />
...
Detta projekt ska utvecklas med TDD. Ni har fått två starter-tester i `test/request_test.rb` som testar de enklaste funktionerna från `get-index.request.txt`.

**Er uppgift är att:**
- Upptäcka vilka fler tester ni behöver skriva
- Bestämma i vilken ordning testerna ska skrivas
- Låta testerna driva er implementation

**Tänk på:**
- Börja med de enklaste testerna först
- Ett test i taget - kör testerna och få dem gröna innan ni skriver nästa test
- Använd alla fyra example-filerna i era tester
- Fundera på edge cases och felhantering

### Katalogstruktur

Organisera er kod enligt följande struktur:

```
lib/
  request.rb          # Er Request-klass
test/
  request_test.rb     # Era tester (två starter-tester finns redan)
  spec_helper.rb
get-index.request.txt
get-examples.request.txt
get-fruits-with-filter.request.txt
post-login.request.txt
```

Kör era tester från projektets rotkatalog med:
```bash
ruby test/request_test.rb
```

### Exempelrequests och förväntade resultat

Nedan följer de fyra exempelfilerna och vad era tester ska verifiera att er `Request`-klass returnerar.
